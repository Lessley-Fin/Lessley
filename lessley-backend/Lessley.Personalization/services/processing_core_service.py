...
class ProcessingCoreService:
    ...
    def get_top_spending_accounts(
        ...
        # 4 . Expenses are negative (e.g., -326). We want the absolute value to calculate total spend.
        analysis_df["sort_column"] = analysis_df["amount.chargedAmount.amount"].abs()

        # 4. Group by the category, sum the amounts, and sort descending
        top_accounts = (
            analysis_df.groupby("accountId")["sort_column"]
            .sum()
            .reset_index()
        ...
        # 5. Take the top 3 and convert back to a standard Python dictionary list
        return top_accounts.to_dict(orient="records")
