...
class UserWallet:
    ...
    # complete field mapping; empty unless a wallet models usage/channel data.
...
def wallet_to_user_context(wallet: UserWallet) -> UserContext:
    ...
    return UserContext(
        ...
        preferred_channels=list(wallet.preferred_channels),
...
def load_wallets(file_path: str | Path) -> dict[str, UserWallet]:
    ...
    for w in raw:
        ...
        wallets[w["user_id"]] = UserWallet(
            ...
            credit_cards=cards,
            preferred_channels=w.get("preferred_channels", []),
