"""Caller identity, as established by the edge.

Ported from ``Lessley.Personalization/dependencies/auth.py``; keep the two in step.

The optimizer does not key any data on the caller — memberships arrive in the request
body — so this establishes *that* there is a logged-in user, not *whose* data to serve.
Caddy's forward_auth is what makes that true: it authenticates every optimizer request
against the Gateway before forwarding it, so an unauthenticated caller never reaches
this service at all. The dependency is the second line of that same defense.
"""
...
from .edge_auth import dev_bypass_active
...
def email_from_access_token(request: Request) -> str | None:
    ...
    bypass already means nothing in this request path is trusted for authorization.
    """
...
def authenticated_email(request: Request) -> str:
    ...
    Never take this from a query parameter or body field — that is the whole reason this
    service can be exposed at the edge at all.
    """
    ...
    logger.warning(
        "Request reached a user-scoped route without a verified identity (path=%s)",
        request.url.path,
    )
    raise HTTPException(
        status_code=401,
