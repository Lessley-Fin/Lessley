...
export function NotificationToast({ type, message, dealId, onView, onDismiss }: NotificationToastProps) {
  ...
  return (
    ...
    >
      ...
      <button type="button" onClick={() => triggerExit(onView)} className="w-full px-4 py-3 text-left transition-colors hover:bg-secondary/60">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1 space-y-1.5">
            ...
            {dealId ? (
              <p className="truncate text-xs text-muted-foreground">Deal · {dealId}</p>
            ) : null}
          </div>
        </div>
      </button>
